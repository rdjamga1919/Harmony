...
if (isset($_POST['type']) && isset($_POST['id_poste'])) {
    ...
    if ($id_commentaire) {
        $stmt = $pdo->prepare("SELECT id_reaction FROM reaction WHERE id_utilisateur = ? AND id_commentaire = ? AND type = ?");
        $stmt->execute([$id_utilisateur, $id_commentaire, $type]);
    } else {
        $stmt = $pdo->prepare("SELECT id_reaction FROM reaction WHERE id_utilisateur = ? AND id_poste = ? AND id_commentaire IS NULL AND type = ?");
    ...
    if ($existante) {
        $stmt2 = $pdo->prepare("DELETE FROM reaction WHERE id_reaction = ?");
        $stmt2->execute([$existante['id_reaction']]);
    } else {
        $stmt2 = $pdo->prepare("INSERT INTO reaction (type, id_utilisateur, id_poste, id_commentaire) VALUES (?, ?, ?, ?)");
